export {};
const { openBingoInput, boardWidthHeightSize, checkOffInputs, checkHorizontalBingo, checkVerticalBingo, calculateBingoBoard} = require('../helper')


const findBingoedBoardIndexes = ( checkedBoards: Array<Array<boolean>> ): Array< number> => {
...
const main = (inputNums: Array<number>, boards: Array<Array<number>>): number => {
  ...
  for(let i = 0; i < inputNums.length; i++){
    ...
    bingoedBoardIndexes.forEach( bingoedIndex => {
      checkedBoardsIndexTracker = checkedBoardsIndexTracker.filter( (index) => index !== bingoedIndex)
    })
...
const present = async () => {
  ...
  console.log('The bingo boards are -', boards.slice(0,3), '\n...\n(only first 3 shown)\n')
  console.log('The numbers in the order they are called are -', inputNums)
  console.log('The result of the sum of all the unmarked numbers on the losing board multiplied by the last called number is-')
