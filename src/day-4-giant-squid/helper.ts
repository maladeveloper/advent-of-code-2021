...
const openBingoInput = async () => {
  ...
  const bingoInput = rawBingoInput.split(/\r?\n/).filter( input => input.length > 0)
  const inputNums = bingoInput.shift().split(/,/).map(input => parseInt(input))
  ...
  for (let i = 0,j = bingoInput.length; i < j; i += boardWidthHeightSize) {
    ...
    const board = seperatedBoard.map( row => row.split(/\s+/)
                                                .filter(boardInput => boardInput.length > 0)
                                  )
                                .flat()
...
module.exports = { openBingoInput, boardWidthHeightSize, checkOffInputs, checkVerticalBingo, checkHorizontalBingo, calculateBingoBoard } 
