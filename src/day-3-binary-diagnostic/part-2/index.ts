...
const findBinaryDigitFrequency = (
  binaryNums: Array<string>,
  index: number
): Array<number> => {
  let zeroCounter = 0;
  let oneCounter = 0;
...
const commonDigitLogic = (
  zeroCounter: number,
  oneCounter: number,
  filterByMostCommon: boolean
): BinaryDigit => {
...
const genericCalculator = (
  ...
  while (finalBinaryHolder.length > 1) {
    ...
    const [zeroCounter, oneCounter] = findBinaryDigitFrequency(
      finalBinaryHolder,
      i
    );

    const filterBinaryDigit = commonDigitLogic(
      zeroCounter,
      oneCounter,
      filterByMostCommon
    );

    finalBinaryHolder = finalBinaryHolder.filter(
      (binaryNum) => binaryNum[i] === filterBinaryDigit
    );
...
const calculateOxyGenRating = (
  binaryNums: Array<string>,
  minBinaryLen: number
): string => genericCalculator(binaryNums, minBinaryLen, true);

const calculateCO2Scrub = (
  binaryNums: Array<string>,
  minBinaryLen: number
): string => genericCalculator(binaryNums, minBinaryLen, false);

const main = (binaryNums: Array<string>): number => {
  const minBinaryLen = Math.min(
    ...binaryNums.map((binaryNum) => binaryNum.length)
  );
